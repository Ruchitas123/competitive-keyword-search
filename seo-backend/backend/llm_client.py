            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if rewritten and len(rewritten) > 50:
                        ...
                        successful += 1
                        print(f"[ContentRewriting] SUCCESS Chunk {chunk_num}: {len(chunk)} -> {len(rewritten)} chars")
                        break
                    else:
                        print(f"[ContentRewriting] WARNING Chunk {chunk_num} attempt {attempt+1}: LLM returned too little")
                    ...
                    error_msg = str(e)[:150]
                    print(f"[ContentRewriting] ERROR Chunk {chunk_num} attempt {attempt+1} error: {error_msg}")
                    
                    if attempt == max_retries - 1:
                        # Last attempt failed, use original with basic HTML
                        print(f"[ContentRewriting] WARNING Using original content for chunk {chunk_num}")
                        rewritten_chunks.append(f"<p>{chunk}</p>")
                    else:
